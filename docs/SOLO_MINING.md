...
| `Block decode failed` on submitblock   | Fixed in **v1.4.21+**: submitblock hex must include empty Verium `vchBlockSig` after transactions. Upgrade miner.                                                                                                       |
| `Block does not start with a coinbase` | Fixed in **v1.4.20+**: GBT coinbase must include Verium's `nTime` field after version. Upgrade the miner; use `--no-getwork` with getblocktemplate.                                                                     |
