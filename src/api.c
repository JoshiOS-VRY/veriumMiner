...
static void cpustatus(int thr_id)
{
	if (thr_id >= 0 && thr_id < opt_n_threads) {
		...
		snprintf(buf, sizeof(buf), "CPU=%d;HPM=%.2f|", thr_id,
			thr_hashrates[thr_id] * 60.0);
...
static char *getsummary(char *params)
{
	...
	*buffer = '\0';
	snprintf(buffer, MYBUFSIZ, "NAME=%s;VER=%s;API=%s;"
		...
		uptime, (uint32_t) ts,
		g_pool_connected, g_pool_status, g_worker_name);
	return buffer;
}

static char *getjson(char *params)
{
	(void)params;
	char *j = stats_json_summary();
	if (!j) {
		*buffer = '\0';
		return buffer;
	}
	snprintf(buffer, MYBUFSIZ, "%s|", j);
	free(j);
	return buffer;
}

static char *gethealth(char *params)
{
	(void)params;
	*buffer = '\0';
	snprintf(buffer, MYBUFSIZ, "STATUS=%s;POOL=%d;ACCPCT=%.1f;TEMP=%.0f|",
		g_pool_connected && stats_accept_pct() > 50.0 ? "ok" : "degraded",
		g_pool_connected, stats_accept_pct(), (double)cpu_temp(0));
	return buffer;
}

static char *getmetrics(char *params)
{
	(void)params;
	const struct topo_info *tp = topo_get();
	*buffer = '\0';
	snprintf(buffer, MYBUFSIZ,
		"verium_hashrate_hps %.2f\n"
		"verium_hashrate_ema60 %.2f\n"
		"verium_hashrate_ema900 %.2f\n"
		"verium_accepted %u\n"
		"verium_rejected %u\n"
		"verium_accept_pct %.2f\n"
		"verium_uptime_sec %ld\n"
		"verium_pool_connected %d\n"
		"verium_temp_celsius %.1f\n"
		"verium_threads %d\n"
		"verium_physical_cpus %d\n|",
		stats_total_hps(), stats_ema_60s(), stats_ema_900s(),
 ...
 * seturl|stratum+tcp://WALLET.WORKER:PASSWORD@pool.example.com:3333|
 * seturl|stratum+tcp://WALLET.WORKER:PASSWORD@backup-pool.example.com:3333|
 */
...
static int websocket_handshake(SOCKETTYPE c, char *result, char *clientkey)
{
	...
	vsha1_init(&ctx);
	vsha1_update(&ctx, inpkey, strlen(inpkey));
	vsha1_final(&ctx, sha1);
