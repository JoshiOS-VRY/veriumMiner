...
void cpuminer_config_dir(char *out, size_t bufsize)
{
#ifdef WIN32
	...
	if (!base || !base[0])
		base = ".";
	snprintf(out, bufsize, "%s\\cpuminer", base);
#else
	const char *home = getenv("HOME");
	if (!home || !home[0])
		home = ".";
	snprintf(out, bufsize, "%s/.cpuminer", home);
#endif
	out[bufsize - 1] = '\0';
}
...
void cpuminer_config_json_path(char *out, size_t bufsize)
{
	...
	cpuminer_config_dir(dir, sizeof(dir));
#ifdef WIN32
	snprintf(out, bufsize, "%s\\cpuminer-conf.json", dir);
#else
	snprintf(out, bufsize, "%s/cpuminer-conf.json", dir);
#endif
	out[bufsize - 1] = '\0';
...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	cpuminer_config_json_path(out, bufsize);
	if (stat(out, &info) == 0) {
		free(cmd);
		return;
	}
...
{
	stats_format_hpm(hashrate, output, 64);
}
